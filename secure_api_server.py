#!/usr/bin/env python3
"""
Enhanced Production API Server with Guardrails
Features:
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(LOG_DIR, f"api_server_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"))
    ]
)
logger = logging.getLogger("strategy-api-secure")

# Project root
PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))

# ===== GUARDRAIL 1: INPUT VALIDATION MODELS =====

class StrategyMetrics(BaseModel):
    """Model for strategy metrics with data validation"""
...
class WeightEntry(BaseModel):
    """Model for strategy weights with data validation"""
...
class UserUpdate(BaseModel):
    """Model for updating user admin endpoints"""
...
class UserManager:
    ...
    def user_to_response(self, user: Dict[str, Any]) -> UserResponse:
        """
...
class ErrorBoundary:
    ...
    async def handle_exceptions(request: Request, call_next):
        ...
        try:
            ...
            # If exception is already HTTP exception, pass it through
            ...
            return JSONResponse(
                ...
                content={
                    ...
                    "errorId": secrets.token_hex(8),  # Generate unique error ID for tracking
...
class AuditLogger:
    ...
    def __init__(self, log_file: str = None):
        ...
        self.log_file = log_file or os.path.join(LOG_DIR, f"audit_{datetime.now().strftime('%Y%m%d')}.log")
    ...
    def log_request(self, request: Request, client: Dict[str, Any], status_code: int):
        """
        Log API request for audit purposes
        
        Args:
            request: FastAPI request
            client: Client info from API key validation
            status_code: HTTP status code
        """
        self.logger.info(
            f"REQUEST | Client: {client['client']} | "
    ...
    def log_data_access(self, client: Dict[str, Any], data_type: str, action: str):
        """
...
class Settings:
    ...
    def __init__(self):
        self.app_name = "Strategy Analysis API (Secure)"
        self.version = "3.0.0"
...
settings = Settings()
rate_limiter = RateLimiter(rate_limit=settings.rate_limit)
...
app = FastAPI(
    ...
    description="Secure API for strategy backtesting analysis and weights",
...
async def guardrail_middleware(request: Request, call_next):
    """Middleware for enforcing guardrails"""
    ...
    # Check rate limit
    if rate_limiter.is_rate_limited(client_identifier):
        logger.warning(f"Rate limit exceeded for client: {client_identifier}")
        
        # Log security event
        audit_logger.log_security_event(
            event_type="RATE_LIMIT_EXCEEDED",
            details=f"Client: {client_identifier}, Path: {request.url.path}",
            severity="WARNING"
        )
        ...
        return JSONResponse(
            ...
            content={
                ...
                "retry-after": "60",
                "errorId": secrets.token_hex(8)
            },
            headers={"Retry-After": "60"}
        )
    
    # Process request
    response = await ErrorBoundary.handle_exceptions(request, call_next)
    
    # Calculate request processing time
    process_time = time.time() - start_time
    
    # Log request (skip health checks to reduce log noise)
...
async def get_summary(
    client: Dict[str, Any] = Depends(security_guard.get_api_key),
    current_user: Optional[Dict[str, Any]] = Depends(user_manager.get_current_active_user)
):
    ...
    if user["mfa_enabled"]:
        ...
        if not user_manager.verify_mfa(user["id"], mfa_code):
            ...
            audit_logger.log_security_event(
                ...
                details=f"User {form_data.username} failed MFA",
                severity="WARNING"
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
...
async def enable_mfa(
    ...
    # Enable MFA
    user_manager.enable_mfa(current_user["id"], mfa_code)
...
async def disable_mfa(
    ...
    # Optionally verify code before disabling
    user_manager.verify_mfa(current_user["id"], mfa_code)
...
@app.get("/v1/admin/users/{username}", response_model=UserResponse)
async def get_user_by_username(
    ...
    audit_logger.log_security_event(
        ...
        details=f"Admin {admin_user['username']} retrieved user {username}",
        severity="INFO"
    )
...
# Admin modify user by username
@app.put("/v1/admin/users/{username}", response_model=UserResponse)
async def update_user(
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
    # Prevent changing self admin status
    if user['id'] == admin_user['id'] and user_update.is_admin is False:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Cannot revoke own admin privileges")
    # Apply updates
    ...
    audit_logger.log_security_event(
        ...
        details=f"Admin {admin_user['username']} updated user {username}",
        severity="INFO"
    )
    return user_manager.user_to_response(user)
...
async def delete_user(
    ...
    # Perform deletion
    del user_manager.users[user['id']]
    # Log admin action
    audit_logger.log_security_event(
        event_type="ADMIN_DELETE_USER",
        details=f"Admin {admin_user['username']} deleted user {username}",
        severity="INFO"
    )
    return {"deleted": True}
...
async def startup_event():
    ...
    audit_logger.log_security_event(
        event_type="STARTUP",
        details=f"API Server started, version={settings.version}",
        severity="INFO"
    )
...
async def shutdown_event():
    ...
    audit_logger.log_security_event(
        event_type="SHUTDOWN",
        details=f"API Server stopped, uptime={time.time() - settings.start_time:.2f}s",
        severity="INFO"
    )
